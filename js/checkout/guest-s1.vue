  ...
  <div>
    <div class="card-body">
      ...
      <div class="form-group row" :class="{'text-danger': form.errors.has('name')}">
          <div class="col-sm-12">
              <input-text id="name" class="mb-0" v-model="form.name" :form="form" v-validate="'required'" 
                          field="name" placeholder="Name on Card"></input-text>
          </div>
      </div>
        ...
        methods: {
            internationalFeeCountryBased(country) {
             ...
             expiry_check(){
                ...
                let ac_year = parseInt(new Date().getFullYear().toString().substr(-2))
            ...
            register() {
                try {
                  ...
                  axios.post('saveStep1', {data: this.form, total_amount: t_a, gold_token: gold_token, checkoutId: this.checkoutId, checkout_details: checkout_details, user: user_ }
                        ...
                        return window.location = "step2?t_a="+t_a+"&gold_token="+gold_token+"&user="+user+"&checkoutId="+checkoutId+"&card_details="+card_details+"&checkout="+checkout;
                    })
                } catch (e) {
