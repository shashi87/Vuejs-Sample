<template>
    <div class="calendar-root">
        <div class="btn btn-light btn-default" @click="toggleCalendar()">
...
<script>
    ...
    export default {
        ...
        created() {
            if (this.isCompact) this.isOpen = true;
            this.dateRange.start = new Date(moment(this.value.start).add(1, 'day').format('YYYY-MM-DD') + 'T00:00:00');
            this.dateRange.end   = new Date(moment(this.value.end).add(1, 'day').format('YYYY-MM-DD') + 'T00:00:00');
        ...
        watch   : {
            ...
            dateRange(value) {
                if (value.start && value.end) this.changed();
            }
        },
        computed: {
        ...
        methods : {
            ...
            getDateString     : function (date, format = this.format) {
                ...
                return moment(dateparse).subtract(1, 'day').format(format);
            },
            ...
            changed() {
                let date = {
                    start: this.dateRange.start ? moment(this.dateRange.start).subtract(1, 'day').format() : null,
                    end  : this.dateRange.end ? moment(this.dateRange.end).subtract(1, 'day').format() : null,
                };
